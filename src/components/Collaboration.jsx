          ...
          <div className="mt-4 relative left-1/2 flex w-[22rem] aspect-square border border-n-6 rounded-full -translate-x-1/2 scale:75 md: scale-100">
            ...
            <ul>
              {collabApps.map((app, index) => (
                <li
                  key={app.id}
                  className={`absolute top-0 left-1/2 h-1/2 -ml-[1.1rem] origin-bottom rotate-${
                    index * 45
                  }`}
                >
                  <div
                    className={`relative -top-1.6rem flex w-[3.2] h-[3.2rem] bg-n-7 border-n-1/15 rounded-xl -rotate-${
                      index * 45
                    }`}
                  >
                    <img
